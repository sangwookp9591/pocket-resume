...
/** 한 장의 에셋. `from`이 있으면 생성하지 않고 다른 장에서 파생합니다. */
export interface Asset {
  /** `char/hero-down` 처럼 디렉터리를 포함한 키. 엔진이 참조하는 이름과 같습니다. */
  id: string;
  /** 저장소 루트 기준 경로. */
  path: string;
  w: number;
  h: number;
  transparent: boolean;
  /** higgsfield aspect_ratio. 파생 에셋에는 없습니다. */
  ar?: string;
  model?: 'nano_banana_flash' | 'gpt_image_2';
  /** 프롬프트의 알맹이. 공통 규격은 promptOf가 붙입니다. */
  subject?: string;
  /** promptOf(a)의 결과. 모듈 로드 시 채워집니다. */
  prompt?: string;
  /** 참조 이미지의 절대 경로. */
  ref?: string | null;
  /** 다른 에셋의 **원본 PNG**를 참조로 씁니다. 그 장이 먼저 나와야 합니다. */
  refFrom?: string;
  /** 이 에셋에서 파생합니다(걷기 2프레임). 크레딧 0. */
  from?: string;
  align?: 'bottom' | 'center';
  /** 타일 발자국의 폭을 채우기 위한 가로 확대 상한. 오브젝트만 1보다 큽니다. */
  stretch?: number;
}
...
const VIEW = {
  ...
  /* 아잉은 2차에서도 얼굴이 정면으로 나왔습니다 — 고양이 얼굴은 모델이 자꾸 정면으로 돌립니다.
     "코가 왼쪽 가장자리를 가리킨다"처럼 기하로 말해야 겨우 돕니다. */
  side: 'turned exactly ninety degrees to the left so we see a strict side profile silhouette, ' +
    'the nose and the face point at the left edge of the frame, only one eye is visible, ' +
    'the far eye is completely hidden behind the head, walking to the left, ' +
    'whole body visible from head to feet',
};

type Dir = keyof typeof VIEW;

const char = (
  name: string, dir: Dir, who: string, ref?: string | null, refFrom?: string,
): Asset => ({
  id: `char/${name}-${dir}`,
...
const walk = (name: string, dir: Dir): Asset => ({
  id: `char/${name}-${dir}-2`,
...
const mon = (id: string, subject: string): Asset => ({
  id: `mon/${id}`,
...
const obj = (id: string, tw: number, th: number, ar: string, subject: string): Asset => ({
  id: `obj/${id}`,
...
const bg = (
  id: string, w: number, h: number, ar: string, model: Asset['model'], subject: string,
): Asset => ({
  id: `bg/${id}`,
...
export function promptOf(a: Asset): string {
  return `${a.subject}, ${STYLE}${a.transparent ? CUTOUT : ''}`;
}

for (const a of ASSETS) if (!a.from) a.prompt = promptOf(a);

/** 계약 7절 단가. gen.ts가 소모 크레딧을 이걸로 셉니다. */
export const PRICE = {
  nano_banana_flash: 1.5,
  gpt_image_2: 7,
  image_background_remover: 0.2,
} as const;

export const byId = (id: string): Asset | undefined => ASSETS.find((a) => a.id === id);
