#!/usr/bin/env node
/* 에셋 생성기. manifest.ts를 읽어 없는 것만 만듭니다.

     node scripts/assets/gen.ts                  # 없는 것 전부
     node scripts/assets/gen.ts --force mon/react char/hero-down
     node scripts/assets/gen.ts --only obj/      # 접두사로 걸러서
     node scripts/assets/gen.ts --repost --only obj/   # 크레딧 0, 후처리만 다시
     node scripts/assets/gen.ts --jobs 1 --dry

   빌드 단계가 없습니다 — Node 25가 .ts를 그대로 실행합니다(타입 스트리핑).
   그래서 타입은 지우면 없어지는 것만 씁니다(enum·파라미터 프로퍼티 금지, erasableSyntaxOnly).
...
import { ASSETS, PRICE, byId, type Asset } from './manifest.ts';

/** post.py가 마지막 줄에 뱉는 것. fill은 불투명 픽셀 비율입니다. */
interface Stat { w: number; h: number; fill: number }

/** log.json 한 행. 그대로 docs/ASSET-REPORT.md의 표가 됩니다. */
interface Row {
  id: string;
  path: string;
  ok: boolean;
...
let spent = 0;
let halted = false;
const log = new Map<string, Row>();

const money = (m?: string): number => PRICE[m as keyof typeof PRICE] ?? 0;
const line = (s: string): void => void process.stdout.write(`${s}\n`);

/** 크레딧을 태우기 직전에 부릅니다. 상한을 넘길 것 같으면 false. */
function charge(model?: string): boolean {
  const c = money(model);
  if (spent + c > STOP) {
    halted = true;
    return false;
...
const exists = (p: string): Promise<boolean> => stat(p).then((s) => s.size > 0, () => false);

const rawOf = (id: string): string => join(RAW, `${id.replace('/', '_')}.png`);

/** higgsfield는 job 배열을 뱉습니다. 첫 결과의 result_url만 씁니다. */
async function generate(a: Asset): Promise<string> {
  ...
  const jobs = JSON.parse(stdout.slice(stdout.indexOf('['))) as
    { result_url?: string; status?: string }[];
...
async function post(src: string, out: string, a: Asset, derive = false): Promise<Stat> {
...
function gate(a: Asset, st: Stat): void {
...
async function makeOne(a: Asset): Promise<void> {
  ...
  if (a.from) {
    const origin = byId(a.from);
    if (!origin) throw new Error(`manifest에 ${a.from}이 없습니다`);
    const src = join(ROOT, origin.path);
  ...
  for (let t = 1; t <= TRIES; t++) {
    ...
    try {
      ...
      err = e as Error;
      line(`✗ ${a.id} — 시도 ${t}/${TRIES} 실패: ${String(err.message).slice(0, 160)}`);
    }
  }
...
const PASS: ((a: Asset) => boolean)[] = [
  (a) => !a.from && !a.refFrom,
  (a) => !a.from && !!a.refFrom,
  (a) => !!a.from,
];

async function pool(items: Asset[], n: number, fn: (a: Asset) => Promise<void>): Promise<void> {
  const q = [...items];
  await Promise.all(Array.from({ length: Math.min(n, q.length) }, async () => {
    while (q.length) {
      ...
      try { await fn(a); } catch (e) {
        ...
        log.set(a.id, { id: a.id, path: a.path, ok: false, attempts: 0, error: msg.slice(0, 300) });
        line(`✗ ${a.id} — ${msg}`);
      }
    }
  }));
...
const prev = await readFile(LOG, 'utf8')
  .then((s) => JSON.parse(s) as { spent: number; rows: Row[] }, () => ({ spent: 0, rows: [] as Row[] }));
const rows: Row[] = [...prev.rows.filter((r) => !log.has(r.id)), ...log.values()];
