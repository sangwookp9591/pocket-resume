...
import { bakeTileAtlas, tileNames } from '../engine/tilegen.ts';
...
export const CHARS = [
  ...
  ['prof', ['down'], false],
  ['npc-senior', ['down'], false],
  ['npc-ace', ['down'], false],
  ['npc-junior', ['down'], false],
  ['npc-lead', ['down'], false],
  ['npc-nurse', ['down'], false],
];

const DIRS_OF = Object.fromEntries(CHARS.map(([n, d]) => [n, d]));
...
export function charKey(name, dir, frame = 0) {
  ...
  const two = CHARS.find(([n]) => n === name)?.[2];
