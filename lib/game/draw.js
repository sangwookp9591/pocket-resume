...
export function drawWorld(r, view, cam, { timeMs = 0 } = {}) {
  ...
  // 5) 파트너 → 플레이어 순. 같은 칸이면 플레이어가 앞
  drawActor(r, 'aing', follower.x, follower.y, follower.dir, follower.frame);
  drawActor(r, 'hero', player.x, player.y, player.dir, player.frame);
...
function drawActor(r, sprite, x, y, dir, frame) {
  const key = charKey(sprite, dir, frame);
  const w = TILE;
  const h = 48;
  r.draw(key, 0, 0, w, h, x * TILE, (y + 1) * TILE - h, w, h, {
    depth: (y + 1) * TILE + 0.5, // 같은 줄의 오브젝트보다 살짝 앞
    flipX: dir === 'right',
  });
}
