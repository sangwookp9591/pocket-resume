...
import { createWorld, rollEncounter, camera, STEP_MS } from '../lib/game/world.js';
import { createState } from '../lib/game/state.js';
...
test('첫 입력은 방향만 바꾸고, 두 번째에 움직인다', () => {
  const w = createWorld('lab', { ...createState(), x: 6, y: 8, dir: 'up' });
  w.press('left');
...
test('벽으로는 못 간다', () => {
  const w = createWorld('lab', { ...createState(), x: 1, y: 1, dir: 'left' });
...
test('물은 ground지만 못 건넌다', () => {
  ...
  const st = { ...createState(), map: 'wave-harbor', x: 10, y: 11, dir: 'left' };
...
test('NPC가 선 칸으로는 못 간다', () => {
  const st = { ...createState(), map: 'lab', x: 9, y: 4, dir: 'up' };
...
test('달리면 같은 칸을 더 빨리 간다', () => {
  const mk = () => createWorld('lab', { ...createState(), x: 6, y: 8, dir: 'left' });
...
test('워프 칸을 밟으면 warp를 낸다', () => {
  const out = [];
  const w = createWorld('lab', { ...createState(), x: 6, y: 9, dir: 'down' }, { onEvent: (e) => out.push(e) });
  walk(w, 'down'); // (6,10) = 문
  assert.equal(out[0]?.kind, 'warp');
  assert.equal(out[0].warp.to, 'newbie-town');
});
...
test('goto는 맵을 갈아 끼우고 enter 이벤트를 낸다', () => {
  ...
  assert.equal(r?.script, 'newbie.arrive');
});

test('A버튼이 바라보는 칸의 NPC를 집는다', () => {
  const w = createWorld('lab', { ...createState(), x: 9, y: 4, dir: 'up' });
  assert.equal(w.interact()?.script, 'lab.prof');
});

test('A버튼이 바라보는 칸의 interact 이벤트를 집는다', () => {
  const w = createWorld('lab', { ...createState(), x: 6, y: 4, dir: 'up' });
  assert.equal(w.interact()?.script, 'lab.balls'); // (6,3) = 책상
});

test('아무것도 없으면 A버튼은 null', () => {
  const w = createWorld('lab', { ...createState(), x: 6, y: 6, dir: 'down' });
  assert.equal(w.interact(), null);
});

test('맵 이벤트의 unless는 "아직 아니라면 발동"이다', () => {
  const before = createWorld('lab', { ...createState(), x: 6, y: 8, dir: 'down' }, {});
  before.press('down'); before.press('down');
  for (let i = 0; i < 30 && before.moving; i++) before.update(40);
  assert.equal(before.takePending()?.script, 'lab.blockExit', '스타터 전인데 안 막았다');
...
test('풀숲에서 인카운터가 나오고, 연속으로는 안 나온다', () => {
  const st = { ...createState(), map: 'newbie-town', x: 3, y: 12, dir: 'down' };
...
test('인카운터 확률이 0이면 절대 안 나온다', () => {
  const st = { ...createState(), map: 'newbie-town', x: 3, y: 10, dir: 'down' };
...
test('파트너가 한 칸 뒤를 따라온다', () => {
  const w = createWorld('night-office', { ...createState(), x: 7, y: 8, dir: 'up' });
