...
| 항목 | 값 | 이유 |
|---|---|---|
| 타일 | **32×32 px** | HGSS와 같음 |
| 논리 해상도 | **512×352** (16×11 타일) | DS(256×192)보다 넓게. 웹에서 답답하지 않은 최소치 |
| 확대 | **정수배만** (1×~4×), `image-rendering: pixelated` | 픽셀 흐림 금지 |
...
`over` 레이어 — 위에 놓이는 것. 대문자는 **충돌**, 소문자는 **통과**.

| 문자 | 오브젝트 | 문자 | 오브젝트 |
|:-:|---|:-:|---|
| `T` | 나무 `tree` | `t` | 작은 나무 `tree-small` |
| `R` | 바위 `rock` | `g` | **풀숲** `bush` (인카운터) |
| `H` | 건물(범례에서 지정) | `D` | 문 = warp 지점 |
...
- 규격: **투명 배경**, 폭은 타일 배수(1~4타일 = 32~128px), 높이는 자유(위로 넘침 허용).
- 정면에 가까운 **약간의 하이앵글**(HGSS의 3/4 시점).

| id | 크기(타일) | 무엇 |
|---|:-:|---|
| `bld-lab` | 4×3 | 아잉 연구소. 흰 벽, 파란 지붕 |
| `bld-house` | 3×2 | 초가지붕 시골집 |
...
- 규격: **1024×704**(=512×352 ×2). 새벽 언덕에서 아잉과 주인공의 뒷모습. 글자 없음.
...
| 배경 제거 ~59 | `image_background_remover` | ~0.2 | 59 | ~12 |
| **소계** | | | **65** | **≈115** |
| 재시도 여유 | | | | ~185 |
| **상한** | | | | **300** |
