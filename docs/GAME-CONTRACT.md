...
| 항목 | 값 | 이유 |
|---|---|---|
| 타일 | **16×16 px** | 인터넷의 CC0 픽셀 팩이 전부 16×16입니다. 여기서 벗어나면 외부 에셋을 한 장도 못 씁니다 |
| 논리 해상도 | **256×176** (16×11 타일) | DS(256×192)와 사실상 같은 크기 |
| 확대 | **정수배만** (2×~5×), `image-rendering: pixelated` | 픽셀 흐림 금지. 기본 3× = 768×528 |
| 캐릭터 | **16×24** (발끝이 타일 하단에 정렬, 위로 8px 넘침) | 머리가 타일을 넘는 HGSS 비율 |
...
| 팩 | 라이선스 | 규격 | 무엇을 가져오나 |
|---|---|---|---|
| [Ninja Adventure Asset Pack](https://pixel-boy.itch.io/ninja-adventure-asset-pack) ([github](https://github.com/pixel-boy/NinjaAdventure)) | **CC0** (저작자 표시 불요, 상업 이용 가능) | 16×16 | 집·나무·덤불·울타리·바위·그루터기, 실내 바닥·벽 |

- CC0라 법적으로 자유롭지만 **`docs/CREDITS.md`에 반드시 출처를 적습니다.** 표시 의무가 없는 것과
  출처를 숨기는 것은 다른 문제입니다.
- 닌텐도/포켓몬의 실제 스프라이트는 **쓰지 않습니다.** 디컴파일 프로젝트(pret/porytiles 등)의 타일도
  마찬가지입니다 — 공개 이력서에 올릴 것이라 IP 리스크를 지지 않습니다. 문법만 빌립니다.
- 팩에 없는 것만 AI로 만듭니다. 아잉·주인공·기술몬 18종은 어느 팩에도 없으므로 AI 몫입니다.
...
- 규격: **16×24**, 투명 배경, 정면 도트. 발끝이 아래 변에 닿게.
- **생성은 384×576(=24배)로 하고 2단계로 줄입니다** — 16×24를 직접 시키면 AI가 뭉갠 것을 뱉습니다.
...
**대부분 Ninja Adventure(CC0)에서 잘라 씁니다.** AI로 만드는 것은 그 팩에 없는 현대 건물뿐입니다.

#### D-1. 팩에서 슬라이스 — `obj/<id>.webp` · 크레딧 0

`tileset_village_abandoned.png`(320×192)와 `tileset_floor.png`에서 16px 격자로 잘라냅니다.
좌표는 `scripts/assets/vendor.mjs`에 표로 적고, 그 표가 곧 출처 기록입니다.

`tree` · `tree-small` · `bush` · `rock` · `fence` · `flower` · `stump` · `sign`
→ 팩의 색이 이끼 낀 폐촌 톤이라 **계약 6절 팔레트 쪽으로 색상 이동**을 한 번 먹입니다(ffmpeg `colorbalance`/`hue`).
한 번의 같은 변환을 전부에 똑같이 걸어야 서로 안 어긋납니다.

#### D-2. AI 생성 — 팩에 없는 것만

| id | 크기(타일) | 무엇 |
|---|:-:|---|
| `bld-lab` | 4×3 | 아잉 연구소. 흰 벽, 파란 지붕 |
| `bld-house` | 3×2 | 시골집 (팩 것으로 대체 가능하면 대체) |
| `bld-office-1` | 4×3 | 낡은 SI 사무실 빌딩 |
| `bld-office-2` | 4×3 | 항구의 유리 사옥 |
| `bld-office-3` | 4×3 | 밝은 스타트업 사옥 |
| `bld-tower` | 5×4 | ZIVO 타워. 유리 고층 |
| `bld-cafe` | 3×2 | 회복 카페(센터). 빨간 지붕 |
| `bld-gym` | 4×3 | 체육관. 돌기둥 |
| `lamp` | 1×2 | 가로등 |
| `mailbox` | 1×1 | 우편함 |
| `desk` | 2×1 | 모니터 놓인 책상 |
...
건물은 타일 크기 × 16px입니다 — `bld-tower`는 80×64. **생성은 1024로 하고 2단계로 줄입니다.**

합계 **AI 13장** (팩 슬라이스 8장은 크레딧 0).

### 5.E 배틀 배경 — `bg/battle-<n>.webp`

- 규격: **256×96**, 불투명. 배틀 화면 상단 절반에 깔립니다.
...
- 규격: **256×176**. 새벽 언덕에서 아잉과 주인공의 뒷모습. 글자 없음. 생성은 1024×704로.
...
| 건물 13 (팩에 없는 것만) | `nano_banana_flash` | 1.5 | 13 | 19.5 |
